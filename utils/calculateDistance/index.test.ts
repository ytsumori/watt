...
describe("calculateDistance", () => {
  it("1km以上の場合", () => {
    ...
    expect(distance.raw).toBe(1699.904987067029);
    expect(distance.formated).toBe("1.7km");
  });
  ...
  it("1km未満の場合", () => {
    ...
    const distance = calculateDistance({ origin, destination });
    expect(distance.raw).toBe(205.48054755948846);
    expect(distance.formated).toBe("205m");
  });
});
